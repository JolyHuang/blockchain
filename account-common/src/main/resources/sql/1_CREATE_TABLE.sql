...
DROP TABLE ADDRESS_NOTICE_TASK;
...
CREATE TABLE ADDRESS_NOTICE_TASK
(
  ID                    CHAR(32) NOT NULL COMMENT 'id',
  ADDRESS_NOTICE_ID     CHAR(32) NOT NULL COMMENT '地址通知id',
  TYPE                  VARCHAR(20) NOT NULL COMMENT '类型(CZCLZ:充值处理中、CZQKQRZ:充值区块确认中、CZCG:充值成功)',
  STATUS                VARCHAR(100) NOT NULL COMMENT '处理状态(WCL:未处理、CLZ:处理中、SUCCESS:处理成功、FAIL:处理失败)',
  CREATE_TIME           TIMESTAMP NULL COMMENT '创建时间',
  MODIFY_TIME           TIMESTAMP NULL COMMENT '修改时间',
  PRIMARY KEY (ID)
);
ALTER TABLE ADDRESS_NOTICE_TASK COMMENT '地址通知任务表';
...
(
  ...
  TX_STATUS             VARCHAR(10) NOT NULL COMMENT '交易是否有效(WCL:未处理、QKQRSCLZ:区块确认数处理中、YEWQR:余额未确认、YEQRYC:余额确认异常、YX:有效、WX:无效)',
